...
//-------------------------------------^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	system("convert ./images/programtext.png ./images/programtext.ppm");
	painImage = ppm6GetImage("./images/programtext.ppm");
	
	
	glGenTextures(1, &painTexture);
	glGenTextures(1, &silhouetteTexture);

	glBindTexture(GL_TEXTURE_2D, painTexture);

	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
	
	unsigned char *silhouetteData = buildAlphaData(painImage);	
...
{
	for(int i = 0; i < 17; i++) {
	if((game->character.center.x + game->character.width > game->walkers[i].center.x-15) && 
		(game->character.center.x- game->character.width < game->walkers[i].center.x +10) && 
		(game->character.center.y - 10 < game->walkers[i].center.y + 10) &&
		(game->character.center.y + 10 > game->walkers[i].center.y - 10)) 
	{
		soundHit();
		game->character.health -= 20;
		//soundHit();
		if(game->character.center.x-10 < game->walkers[i].center.x+10) 
		{
...
{ 
	if(walkersRight == 1) {
		for(int i = 0; i < 17; i++) {
		game->walkers[i].center.x += 1;
		//game->walkers[1].center.x += 1;
		}
		}
	if(walkersRight == 0) {
		for(int i = 0; i < 17; i++) {
...
void enemyReverse(Game *game)
{	
	if(game->walkers[0].center.x < -1040)
		walkersRight = 1;
	if(game->walkers[0].center.x > -960)
		walkersRight = 0;
